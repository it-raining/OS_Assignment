...
{
  if (pre != 0) {   // Neu trang hien dang duoc su dung / hien? thi
    if (swp == 0) { // Non swap ~ page online
      ...
      CLRBIT(*pte, PAGING_PTE_SWAPPED_MASK); // Dua page tro lai mem
      CLRBIT(*pte, PAGING_PTE_DIRTY_MASK);

      SETVAL(*pte, fpn, PAGING_PTE_FPN_MASK,
             PAGING_PTE_FPN_LOBIT); // Set FPN = fpn
    } else {                        // page swapped
      SETBIT(*pte, PAGING_PTE_PRESENT_MASK);
      SETBIT(*pte, PAGING_PTE_SWAPPED_MASK); // Dang o vmem
      CLRBIT(*pte, PAGING_PTE_DIRTY_MASK);

      SETVAL(*pte, swptyp, PAGING_PTE_SWPTYP_MASK,
             PAGING_PTE_SWPTYP_LOBIT); // Set SWPTYP = swpty
      SETVAL(*pte, swpoff, PAGING_PTE_SWPOFF_MASK,
             PAGING_PTE_SWPOFF_LOBIT); // Set SWPOFF = swpoff
    }
  }
 ...
 * @swpoff : swap offset
 */
int pte_set_swap(uint32_t *pte, int swptyp, int swpoff) // swapped out
{
  SETBIT(*pte, PAGING_PTE_PRESENT_MASK);
 ...
 */
int pte_set_fpn(uint32_t *pte, int fpn) // swapped in
{
  SETBIT(*pte, PAGING_PTE_PRESENT_MASK);
 ...
 */
int vmap_page_range(
    struct pcb_t *caller,           // process call (PCB)
